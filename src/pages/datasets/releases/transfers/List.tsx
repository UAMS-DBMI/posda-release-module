...
export default function DatasetReleaseTransfersList() {
  ...
  async function loadTransfers(id: string) {
    const res = await fetch(`/papi/v1/distribution/datasets/releases/${id}/transfers`, { cache: "no-store" });
    ...
    async function load() {
      ...
      try {
        const [releaseRes, transfersRes] = await Promise.all([
          ...
          fetch(`/papi/v1/distribution/datasets/releases/${releaseId}/transfers`, { cache: "no-store" }),
        ]);
  ...
  async function syncTransfers() {
    ...
    try {
      const destRes = await fetch(`/papi/v1/distribution/datasets/releases/${releaseId}/destinations`, { cache: "no-store" });
      ...
      const results = await Promise.allSettled(
        destinations.map(async (dest) => {
          const rsRes = await fetch(
            `/papi/v1/distribution/datasets/releases/${releaseId}/recordsets?destination_id=${dest.destination_id}`,
            { cache: "no-store" },
          );
          ...
          if (!existing) {
            ...
            const currentRes = await fetch(
              `/papi/v1/distribution/transfers/${existing.dataset_release_transfer_id}/recordsets`,
              { cache: "no-store" },
            );
            ...
            await Promise.all([
              toAdd.length > 0
                ? fetch(`/papi/v1/distribution/transfers/${existing.dataset_release_transfer_id}/recordsets/add`, {
                    method: "POST",
                    headers: { "content-type": "application/json" },
                ...
                : Promise.resolve(),
              toRemove.length > 0
                ? fetch(`/papi/v1/distribution/transfers/${existing.dataset_release_transfer_id}/recordsets/remove`, {
                    method: "POST",
                    headers: { "content-type": "application/json" },
