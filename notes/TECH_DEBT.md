...
| 17 | Transfer mode | `transfer_mode` still exists as a table, an FK on both `dataset_release_transfer` and `recordset_destination` (both NOT NULL), and a `/lookups/transfer-modes` endpoint — but the concept was **hidden from curators on 2026-07-24** and is no longer displayed anywhere. The frontend derives it from `destination_abbr` via the hardcoded `transferModeIdForDestination` (`lib/recordsetForm.ts`): `idc`/`gc`/`nbia` → grouped bundle, `wp`/`asp` → single dataset. So a per-destination constant is stored per-row in two tables and looked up through a join that can never vary. `clinical update` is dead lookup data. **Direction (2026-08-24): eliminate the concept and let grouping be a property of the transfer destination.** | Hiding it was a UI fix; removing it is a schema change, and nothing depended on it urgently. Re-surfaced by accident in the Transfer stage (6.2) and removed again — which is exactly the drift a live-but-invisible column invites. | Move grouping onto `transfer_destination` (a column there, or drop the notion entirely if the per-destination handler already implies it), drop `transfer_mode` + both FKs, retire `/lookups/transfer-modes` and `transferModeIdForDestination`, and strip `transfer_mode_id` from `DatasetReleaseTransferInsert`/`Update` and `update_recordset_destination`. Touches all three SQL scripts (add/drop/test-data) plus `transferForm.ts`, `recordsetDestinations.ts`, `recordsetForm.ts`, and the three standalone transfer pages that still declare `transfer_mode_name`. |

## Resolved

- **2026-08-25 — item 5, release gating.** The Transfer stage hid its controls
  while a dataset release was a draft, but the endpoints stayed permissive, so
  the API could be used around the rule. Both halves are now enforced
  server-side in `distribution.py`:
  - `POST /datasets/releases/{id}/transfers` → **422** when the release is
    `draft` (and 404 when it doesn't exist, which it previously never checked).
    A draft's membership can still change under a transfer that has already
    snapshotted it.
  - `PUT /transfers/{id}` → **422** when setting `transfer_status = 'queued'`
    while the parent release is `draft`. This is the one-way door:
    `notify_transfer_queued()` starts real uploads with no route back, so it
    must not be reachable before the contents are settled. The guard only runs
    for the `queued` transition; every other field update is unaffected.
  Both guard queries verified against the local DB. No existing transfer belongs
  to a draft release, so nothing currently valid becomes invalid.
